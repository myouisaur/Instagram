...
// @version      2.7
// @description  Replaces profile grid Reels with an interactive, solid-color placeholder to completely kill the curiosity gap.
// @author       Xiv
// @match        *://*.instagram.com/*
...
(function() {
    ...
    const CONFIG = {
        DEBUG: false,
        STORAGE_KEY: 'xiv_revealed_reels_session',
        SHORTCUT_KEY: 'h', // Alt + H

        SELECTORS: {
            MAIN_CONTAINER: 'main, div[role="main"]',
            NEW_REELS: ':is(main, div[role="main"]) a:is([href*="/reel/"], [href*="/reels/"]):has(img, video):not([data-xiv-overlay-injected])',
            ALL_INJECTED_REELS: 'a[data-xiv-overlay-injected="true"]'
        },
    ...
    const syncThemeState = () => {
        ...
        if (theme !== html.dataset.xivTheme) {
            if (theme) html.dataset.xivTheme = theme;
            else delete html.dataset.xivTheme;
    ...
    const isProfileGridRoute = () => {
        ...
        // Ensure we are exactly at /username/ (one path segment only).
        // This naturally excludes /username/reels/, /username/tagged/, and /reels/12345/
        if (parts.length === 1) {
            const reservedRoots = ['explore', 'direct', 'stories', 'reels', 'about', 'developer', 'language'];
            return !reservedRoots.includes(parts[0]);
        }
    ...
    const updateRouteState = () => {
        ...
        if (isProfileGridRoute()) {
            document.documentElement.dataset.xivHideReels = 'true';
            connectGridObserver();
            ...
            delete document.documentElement.dataset.xivHideReels;
        }
    };
    ...
    const toggleReelState = (reelLink, forceHidden = false, skipSessionUpdate = false) => {
        const isCurrentlyHidden = reelLink.dataset.xivReelState === 'hidden';
        const willBeHidden = forceHidden !== null ? forceHidden : !isCurrentlyHidden;

        reelLink.dataset.xivReelState = willBeHidden ? 'hidden' : 'revealed';

        const btn = reelLink.querySelector('.xiv-reel-toggle-btn');
    ...
    const scanForReels = () => {
        if (document.documentElement.dataset.xivHideReels !== 'true') return;
    ...
    const initKeyboardShortcuts = () => {
        document.addEventListener('keydown', (e) => {
            if (e.altKey && e.key.toLowerCase() === CONFIG.SHORTCUT_KEY) {
                ...
                let hiddenCount = 0;
                allReels.forEach(r => { if (r.dataset.xivReelState === 'hidden') hiddenCount++; });
    ...
    // =========================================================
    const injectStyles = () => {
        if (document.getElementById('xiv-reel-declutter-style')) return;

        const style = document.createElement('style');
        style.id = 'xiv-reel-declutter-style';

        style.textContent = `
            /* --- DEFAULT (LIGHT MODE) --- */
            :root {
                --xiv-reel-cover-bg: ${CONFIG.THEME.LIGHT.COVER_BG};
                --xiv-reel-btn-bg: ${CONFIG.THEME.LIGHT.BTN_BG};
                --xiv-reel-btn-text: ${CONFIG.THEME.LIGHT.BTN_TEXT};
                --xiv-reel-btn-border: ${CONFIG.THEME.LIGHT.BTN_BORDER};
            }

            /* --- OS DARK MODE (Fallback) --- */
            @media (prefers-color-scheme: dark) {
                html:not([data-xiv-theme="light"]) {
                    --xiv-reel-cover-bg: ${CONFIG.THEME.DARK.COVER_BG};
                    --xiv-reel-btn-bg: ${CONFIG.THEME.DARK.BTN_BG};
                    --xiv-reel-btn-text: ${CONFIG.THEME.DARK.BTN_TEXT};
                    --xiv-reel-btn-border: ${CONFIG.THEME.DARK.BTN_BORDER};
                }
            }

            /* --- MANUAL DARK MODE --- */
            html[data-xiv-theme="dark"] {
                --xiv-reel-cover-bg: ${CONFIG.THEME.DARK.COVER_BG} !important;
                --xiv-reel-btn-bg: ${CONFIG.THEME.DARK.BTN_BG} !important;
                --xiv-reel-btn-text: ${CONFIG.THEME.DARK.BTN_TEXT} !important;
                --xiv-reel-btn-border: ${CONFIG.THEME.DARK.BTN_BORDER} !important;
            }

            /* Scoped using :is() to prevent comma selector leakage */
            html[data-xiv-hide-reels="true"] :is(${CONFIG.SELECTORS.MAIN_CONTAINER}) a:is([href*="/reel/"], [href*="/reels/"]):has(img, video) {
                position: relative !important;
                display: block;
            }

            /* --- SOLID COVER LAYER --- */
            .xiv-reel-cover-layer {
                position: absolute;
                inset: 0;
                background-color: var(--xiv-reel-cover-bg);
                z-index: 10;
            ...
            .xiv-reel-toggle-btn {
                ...
                width: 2.25rem;
                height: 2.25rem;
                background: var(--xiv-reel-btn-bg);
                color: var(--xiv-reel-btn-text);
                border: 1px solid var(--xiv-reel-btn-border);
            ...
            .xiv-reel-toggle-btn:hover {
                transform: scale(1.1);
                filter: brightness(1.2);
            }

            /* --- STATE: HIDDEN --- */
            html[data-xiv-hide-reels="true"] a[data-xiv-reel-state="hidden"] .xiv-reel-cover-layer {
                opacity: 1;
                pointer-events: auto;
            }

            html[data-xiv-hide-reels="true"] a[data-xiv-reel-state="hidden"] .xiv-reel-toggle-btn {
                opacity: 1;
            }

            /* --- STATE: REVEALED --- */
            html[data-xiv-hide-reels="true"] a[data-xiv-reel-state="revealed"] .xiv-reel-cover-layer {
                opacity: 0;
                pointer-events: none;
            }

            html[data-xiv-hide-reels="true"] a[data-xiv-reel-state="revealed"] .xiv-reel-toggle-btn {
                opacity: 0.4;
            }

            html[data-xiv-hide-reels="true"] a[data-xiv-reel-state="revealed"] .xiv-reel-toggle-btn:hover {
                opacity: 1;
            }

            /* Disable completely when off route */
            html:not([data-xiv-hide-reels="true"]) .xiv-reel-cover-layer,
            html:not([data-xiv-hide-reels="true"]) .xiv-reel-toggle-btn {
